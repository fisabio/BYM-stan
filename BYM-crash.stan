...
transformed parameters {
  ...
  vector[N] phi = phi_std * sigma_phi;    // non-centered parameterization
}
model {
  //  y ~ poisson_log(log_E + beta0 + beta1 * x + theta + phi);
  
  y ~ poisson_log(log_E + beta0 + theta + phi);
...
generated quantities {
  ...
  vector[N] mu = exp(log_E + beta0 + phi + theta);
  vector[N] SMR = exp(beta0 + phi + theta);
